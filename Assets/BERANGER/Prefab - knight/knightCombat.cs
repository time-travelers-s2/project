...
public class knightCombat : MonoBehaviour
{
    ...
    private void Update() 
    {
        if (knightController.canMove && knightController.grounded && Input.GetKeyDown(attackKey))
        {
            Attack();
        }
    }
    ...
    {   
        ...
        Invoke(nameof(DisableAttack), 1.25f * 1.5f); // Adjust time based on animation
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (isAttacking && ((1 << other.gameObject.layer) & enemyLayers) != 0)
        {
            ...
            if (!hitEnemies.Contains(other)) // Prevent multiple hits on the same enemy
            {
                ...
                if (enemyHealth != null)
                {
                    enemyHealth.GetDamage(20); // Example damage value
                }
                */
            }
        }
